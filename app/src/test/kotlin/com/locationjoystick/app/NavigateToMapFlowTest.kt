...
class NavigateToMapFlowTest {
    ...
    @Test
    fun `navigate_to_map true emits on flow`() = runTest {
    ...
    @Test
    fun `navigate_to_map false does not emit`() = runTest {
        val flow = MutableSharedFlow<Unit>(replay = 1)
        val intent = mockk<Intent>()
    ...
    @Test
    fun `null intent does not emit`() = runTest {
        val flow = MutableSharedFlow<Unit>(replay = 1)
    ...
    @Test
    fun `replay buffer delivers cold-start emission to late collector`() = runTest {
        val flow = MutableSharedFlow<Unit>(replay = 1)
        val intent = mockk<Intent>()
        every { intent.getBooleanExtra(MainActivity.EXTRA_NAVIGATE_TO_MAP, false) } returns true
    ...
    @Test
    fun `multiple handleIntent calls only buffer one replay event`() = runTest {
        val flow = MutableSharedFlow<Unit>(replay = 1)
        val intent = mockk<Intent>()
        every { intent.getBooleanExtra(MainActivity.EXTRA_NAVIGATE_TO_MAP, false) } returns true
