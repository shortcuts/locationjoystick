...
The Capture screen is a compact numbered setup plus the captured-point list:

- **1. Capture mode + List / Jump** — one overall DataStore switch followed by two independent
...
- **2. Set this app as default browser** — link on **default browser** opens Default apps
...
- **3. Turn off Google Maps supported links** — only **Google Maps supported links** is the
...
- **4. Turn on supported links for this app** — only **supported links for this app** is the
  link (this app's "Open by default" screen). Same "verb is not the link" rule as step 3.
- **5. When you are done, restore default browser and reverse steps 3–4** — link on
  **restore default browser** (`ACTION_MANAGE_DEFAULT_APPS_SETTINGS`). Android will not assign
  ...
  green marker color as completed steps but keeps the number **5** (not a check) so restore
  is easy to spot.
- Ready banner when Capture mode and either action are on **and** this app is the default browser.
  Its text explains whether links will be listed, jumped to, or both. A concise pass-through banner
  appears while the overall mode is off or neither action is selected. The pass-through browser
  row opens an in-app browser picker.
...
- Point order defaults to **Optimize proximity** (`orderedCapturedPoints` → `orderByProximity`,
  ...
  **Last** (remove last) is unambiguous; proximity is applied when copying or saving the route.
  Setup steps 2–5 use extra vertical padding so the setting links are harder to mis-tap.
...
the Capture page resumes after a Settings change.
