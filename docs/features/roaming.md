...
- **Route cache**: an in-memory LRU (`OsrmConstants.CACHE_MAX_ENTRIES`, 64) keyed by requested profile plus waypoints rounded to ~1 m (`CACHE_COORD_SCALE`) serves an identical repeat request without a network call for `CACHE_TTL_MS` (1 h). Only a successful ladder result is cached, never a bisected or straight-line-patched one. Once expired, an entry is served only when the ladder fails. Not persisted to disk.
- **Backend cooldown**: HTTP 429 cools the whole host (`Retry-After` if numeric, else `COOLDOWN_RATE_LIMITED_MS` 60 s); HTTP 5xx cools only that backend base URL (`COOLDOWN_SERVER_ERROR_MS` 30 s, since `/routed-foot` etc. are independent services). Any cooldown is capped at `COOLDOWN_MAX_MS` (5 min). Expiry is persisted (`BackendCooldowns`, SharedPreferences `osrm_cooldowns`) so a restart does not re-hit a throttling server. A cooling slot is skipped without a request or ladder wait, including during bisection and `snapToRoad`; if every slot is cooling the request fails immediately and callers fall back to a straight line.
